...
class ExternalApplyEventMethodPayloadsValidatorTest {
  ...
  void throwsWhenPayloadClassDiffersFromRegistration() {
    assertThatThrownBy(() -> applyEventMethodPayloadsValidator.validate("car", WrongPayloadClassEntity.class))
        ...
        .hasMessage("@ApplyEvent Event parameter's payload class must be of type dev.baitursinov.caravan.event.sourcing.applying.ExternalApplyEventMethodPayloadsValidatorTest$CarTurnedOnPayload, which is not the case for dev.baitursinov.caravan.event.sourcing.applying.ExternalApplyEventMethodPayloadsValidatorTest$WrongPayloadClassEntityEventApplier.applyTurnedOn");
  }
  ...
  void failsFastOnStructurallyBrokenEntities() {
    assertThatThrownBy(() -> applyEventMethodPayloadsValidator.validate("car", StructurallyBrokenEntity.class))
        .isInstanceOf(EventSourcedEntitySetupException.class)
        .hasMessage("@ApplyEvent method for eventName=turned-on is duplicated in @EventApplier class dev.baitursinov.caravan.event.sourcing.applying.ExternalApplyEventMethodPayloadsValidatorTest$StructurallyBrokenEntityEventApplier");
  }
}
